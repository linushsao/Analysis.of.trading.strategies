...
while(TRUE)
{
    ...
    price.average <- mean(na.omit(sample.stock$price[,5][row.length-(250*3): row.length]))
    print(paste(sample.stock$code, sample.stock$name, price.average))
    if( (price.average > 25) || !is.null(stock.code.custom)) break
}
...
t.return$trade.earn <- (t.return$ret * t.return$trade.signal.softmax)

result <- t.return$trade.earn[period]
result$return <- cumprod(1 + (result$trade.earn[period]))
...
x11()
par(mfrow=c(3,1))
plot(t.return$Close[period], main=title)
plot(result$return)
plot(t.return$trade.signal.softmax[period])
