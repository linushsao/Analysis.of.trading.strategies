...
#Basic Configure >>
target.path <- "/home/linus/Project/9.Shared.Data/1_Taiwan/finance.yahoo.com/Dividend.policy/"

#Param Configure <<
url.head <- "https://tw.stock.yahoo.com/d/s/dividend_"
url.tail <- ".html"
# crawl.check <- ".crawl.check.csv"
prefix.dp <- "dp"
extension <- ".TW"
file.extension <- ".csv"
crawl.delay.seconds <- c(1:3) #default
fake.header <- ""
...
main <- function() {
    ...
    for (i in 1:length(list.code))  { 
        ...
        if(i > processed.code ) {
            ...
            destfile <- m_paste(c(target.path, prefix.dp, target.stock, file.extension),op="")
